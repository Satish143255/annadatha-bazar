  ...
  return (
    <div className="scroll">
        ...
        <div style={{ padding: "0 16px 24px" }}>
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            ...
            {filtered.map((p, i) => {
              ...
              const emoji = AGRI_DATA.CROPS.find(c => p.commodity.toLowerCase().includes(c.id))?.emoji || "🌾";
              return (
                <div
                  key={p.commodity}
          ...
          <div style={{ fontSize: 11, color: "var(--ink-3)", marginTop: 10, textAlign: "center" }}>
            All prices in ₹/Quintal · Tap row for details
          </div>
        </div>
      )}
            ...
            <div key={p.commodity} className="card" style={{ marginBottom: 10, padding: 0, overflow: "hidden", borderColor: matched ? "var(--primary-soft)" : "var(--border)" }}>
              ...
              >
                <div style={{
                  ...
                  {AGRI_DATA.CROPS.find(c => p.commodity.toLowerCase().includes(c.id))?.emoji || "🌾"}
                </div>
                <div style={{ flex: 1 }}>
      ...
      <div style={{ padding: "0 16px 28px", fontSize: 11, color: "var(--ink-3)", textAlign: "center" }}>
        <Icon name="refresh" size={12} /> Updated 6 hrs ago · Source: Agmarknet
      </div>
    </div>
  );
};
    ...
    <div className="scroll">
      ...
      <div style={{ padding: "8px 16px 16px" }}>
        <div style={{
          ...
          <div style={{ fontSize: 12, opacity: 0.7, marginTop: 2 }}>Feels like {w.current.feelsLike}°</div>
          ...
          {w.daily.map((d, i) => (
            <div key={i} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ fontSize: 11, color: "var(--ink-3)" }}>{d.low}°</span>
                ...
                <span style={{ fontSize: 13, fontWeight: 500 }}>{d.high}°</span>
              </div>
      ...
      {active && (
        <div onClick={() => onOpenListing(active)} style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: "var(--ink-3)" }}>{active.distance}km · {formatINR(active.price)}/{active.priceUnit}</div>
          </div>
...
Object.assign(window, { PricesScreen, WeatherScreen, NearbyScreen });
